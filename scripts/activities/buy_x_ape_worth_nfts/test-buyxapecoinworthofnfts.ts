// scripts/activities/buy_x_apecoin_worth_of_nfts/test-buyxapecoinworthofnfts.ts
import { ethers } from "hardhat";
import * as fs from "fs";
import * as path from "path";

async function main() {
  console.log("\n=== Testing BuyXApecoinWorthOfNFTs with TokenAirdropReward ===");
  ...
  const instanceFile = path.join(outputDir, "ape-nft-purchase-instance.json");
  ...
  console.log(`- APE NFT Purchase Tim3cap: ${instanceData.apeNftPurchaseInstance.tim3cap}`);
  ...
  const BuyXApecoinWorthOfNFTs = await ethers.getContractFactory("BuyXApecoinWorthOfNFTs");
  const TokenAirdropReward = await ethers.getContractFactory("TokenAirdropReward");
  const TestNFT = await ethers.getContractFactory("TestNFT");
  const TestToken = await ethers.getContractFactory("TestToken");
  
  // Attach to deployed contracts
  const apeTim3cap = Tim3cap.attach(instanceData.apeNftPurchaseInstance.tim3cap);
  const apeActivity = BuyXApecoinWorthOfNFTs.attach(instanceData.apeNftPurchaseInstance.activity);
  const apeReward = TokenAirdropReward.attach(instanceData.apeNftPurchaseInstance.reward);
  ...
  // 1. Test BuyXApecoinWorthOfNFTs Activity
  console.log("\n=== Testing BuyXApecoinWorthOfNFTs Activity ===");
  
  // Get Activity Config
  const apeConfig = await apeActivity.getConfig();
  console.log("BuyXApecoinWorthOfNFTs Config:");
  console.log(`- NFT Address: ${apeConfig[0]}`);
  ...
  // Check NFT balance of deployer
  const deployerNftBalance = await testNFT.balanceOf(deployer.address);
  console.log(`Deployer NFT balance: ${deployerNftBalance}`);
  
  // Check initial APE spent and NFT count
  const initialPurchaseInfo = await apeActivity.getUserPurchaseInfo(deployer.address);
  console.log(`Initial APE spent for deployer: ${ethers.formatUnits(initialPurchaseInfo[0], 18)}`);
  console.log(`Initial NFT count for deployer: ${initialPurchaseInfo[1]}`);
  ...
  if (!isEligibleBeforePurchase) {
    console.log("Registering APE NFT purchases to make deployer eligible...");
    ...
    try {
      ...
      await apeActivity.verifyPurchase(
        ...
        1, // Register 1 NFT purchased
        "0x" // Empty proof since we're the owner
      );
      console.log(`Verified purchase of ${ethers.formatUnits(requiredAmount, 18)} APE worth of NFTs (1 NFT) for deployer`);
      
      // Check updated APE spent amount and NFT count
      const updatedPurchaseInfo = await apeActivity.getUserPurchaseInfo(deployer.address);
      console.log(`Updated APE spent for deployer: ${ethers.formatUnits(updatedPurchaseInfo[0], 18)}`);
      console.log(`Updated NFT count for deployer: ${updatedPurchaseInfo[1]}`);
  ...
  // Test APE NFT purchase activity eligibility and claim
  const canClaimReward = await apeReward.canClaim(deployer.address);
  console.log(`Can deployer claim APE NFT purchase reward: ${canClaimReward}`);
  
  if (canClaimReward) {
    // Create eligibility proof for the claim
    console.log("Creating eligibility proof for APE NFT purchase activity...");
    const timestamp = Math.floor(Date.now() / 1000);
    const message = ethers.getBytes(ethers.keccak256(
      ethers.solidityPacked(
        ["address", "uint256", "string"],
        [deployer.address, timestamp, "BUY_X_APECOIN_WORTH_OF_NFTS"]
      )
    ));
    ...
    // Claim from the APE NFT purchase activity instance through Tim3cap
    console.log("Claiming from APE NFT purchase activity instance through Tim3cap...");
    try {
      const apeClaimTx = await apeTim3cap.claim(proof, 0, [], {
        gasLimit: 3000000
      });
      console.log(`APE NFT purchase claim transaction hash: ${apeClaimTx.hash}`);
      
      const apeClaimReceipt = await apeClaimTx.wait();
      console.log(`APE NFT purchase claim successful! Gas used: ${apeClaimReceipt?.gasUsed.toString()}`);
      ...
      console.log("❌ APE NFT purchase claim failed:", error.message);
    }
  } else {
    console.log("Deployer cannot claim APE NFT purchase activity reward (may have already claimed)");
  }
  
  // Test setting signing key and proof validation
  console.log("\n=== Testing Proof Validation ===");
  
  // Update signing key on APE NFT purchase activity
  console.log("Updating signing key on APE NFT purchase activity...");
  ...
  const purchaseMessage = ethers.getBytes(ethers.keccak256(
    ethers.solidityPacked(
      ["address", "uint256", "string"],
      [deployer.address, purchaseTimestamp, "BUY_X_APECOIN_WORTH_OF_NFTS"]
    )
  ));
  ...
  if (isPurchaseProofValid) {
    try {
      // Get current APE spent amount and NFT count
      const beforeAdditionalPurchase = await apeActivity.getUserPurchaseInfo(deployer.address);
      console.log(`APE spent before additional verification: ${ethers.formatUnits(beforeAdditionalPurchase[0], 18)}`);
      console.log(`NFT count before additional verification: ${beforeAdditionalPurchase[1]}`);
      ...
      await apeActivity.verifyPurchase(
        ...
        1, // Register 1 more NFT
        purchaseProof // Using the proof since we're not the owner anymore
      );
      console.log(`Additional purchase of ${ethers.formatUnits(minPurchaseAmount, 18)} APE worth of NFTs (1 NFT) verified successfully`);
      
      // Check updated APE spent amount and NFT count
      const afterAdditionalPurchase = await apeActivity.getUserPurchaseInfo(deployer.address);
      console.log(`APE spent after additional verification: ${ethers.formatUnits(afterAdditionalPurchase[0], 18)}`);
      console.log(`NFT count after additional verification: ${afterAdditionalPurchase[1]}`);
  ...
  console.log("BuyXApecoinWorthOfNFTs Testing Summary:");
  
  const finalPurchaseInfo = await apeActivity.getUserPurchaseInfo(deployer.address);
  ...
  console.log(`- Final APE spent amount: ${ethers.formatUnits(finalPurchaseInfo[0], 18)}`);
  console.log(`- Final NFT count: ${finalPurchaseInfo[1]}`);
