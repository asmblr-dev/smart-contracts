// scripts/activities/buy_x_apecoin_worth_nfts/test-merkle-discount.ts
...
async function main() {
  console.log("\n=== Testing Merkle Discount Proofs with BuyXApecoinWorthOfNFTs ===");
  ...
  console.log(`- Tim3cap Instance: ${instanceData.apeNftPurchaseInstance.tim3cap}`);
  console.log(`- Activity: ${instanceData.apeNftPurchaseInstance.activity}`);
  ...
  const tim3cap = Tim3cap.attach(instanceData.apeNftPurchaseInstance.tim3cap);
  const activity = BuyXApecoinWorthOfNFTs.attach(instanceData.apeNftPurchaseInstance.activity);
  ...
  // Mint NFTs to the deployer (we'll need a few for verification)
  try {
    ...
    console.log(`Note: Could not mint NFT to deployer: ${error.message}`);
  }
  ...
  const registerPurchase = async (user: any) => {
    ...
    console.log(`Registering purchase of ${ethers.formatUnits(requiredAmount, 18)} APE worth of NFTs for ${user.address}`);
    ...
    try {
      ...
      // Check updated purchase info
      const purchaseInfo = await activity.getUserPurchaseInfo(user.address);
      console.log(`Updated APE spent: ${ethers.formatUnits(purchaseInfo[0], 18)}`);
      console.log(`Updated NFT count: ${purchaseInfo[1]}`);
  ...
  // Make sure deployer is eligible first
  ...
  const testDiscountClaim = async (user: any, discountRate: number) => {
    ...
    const message = ethers.getBytes(ethers.keccak256(
      ethers.solidityPacked(
        ...
        [userAddress, timestamp, "BUY_X_APECOIN_WORTH_OF_NFTS"]
      )
    ));
    ...
    try {
      ...
      // We'll use the deployer to claim for simplicity
      ...
      // If the claim failed, check if it's because the user already claimed
      try {
        const canClaim = await activity.canClaim(userAddress);
        console.log(`Can claim check result: ${canClaim}`);
      } catch (error: any) {
        console.log(`Note: Could not check canClaim: ${error.message}`);
      }
    }
  };
  ...
  // Test with user1's discount rate if possible
  await registerPurchase(user1);
  await testDiscountClaim(user1, 2000); // 20%
