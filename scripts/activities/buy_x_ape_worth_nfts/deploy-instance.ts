// scripts/activities/buy_x_apecoin_worth_of_nfts/deploy-instance.ts
import { ethers } from "hardhat";
import * as fs from "fs";
import * as path from "path";

async function main() {
  console.log("\n=== Creating Tim3cap Instance with BuyXApecoinWorthOfNFTs Activity ===");
  ...
  console.log(`- BuyXApecoinWorthOfNFTs Implementation: ${contracts.buyXApecoinWorthOfNFTsImpl}`);
  ...
  // Mint APE and NFTs to the deployer
  ...
  // Prepare activity config - 6 parameter structure for BuyXApecoinWorthOfNFTs
  const activityConfig = ethers.AbiCoder.defaultAbiCoder().encode(
    ["address", "address", "uint256", "uint256", "uint256", "uint256"],
    [
      contracts.testNFT,           // NFT address
  ...
  const tokenRewardConfig = ethers.AbiCoder.defaultAbiCoder().encode(
    ...
    [
      "APE NFT Purchase Rewards",    // reward name
  ...
  // Create Tim3cap instance with BuyXApecoinWorthOfNFTs via Factory
  console.log("\nCreating Tim3cap instance with BuyXApecoinWorthOfNFTs activity via Factory...");
  
  try {
    const createNftTx = await tim3capFactory.createTim3cap(
      "BUY_X_APECOIN_WORTH_OF_NFTS",   // Activity type
      contracts.buyXApecoinWorthOfNFTsImpl, // Activity implementation
    ...
    console.log(`Create transaction hash: ${createNftTx.hash}`);
    console.log("Waiting for transaction confirmation...");
    const nftReceipt = await createNftTx.wait();
    console.log("✅ Transaction successful!");
    
    // Find the Tim3cap address from the event
    const nftCreationEvent = nftReceipt?.logs.find((log: any) => {
      try {
    ...
    const nftParsedEvent = tim3capFactory.interface.parseLog({
      topics: nftCreationEvent.topics,
      data: nftCreationEvent.data
    });
    
    const nftTim3capAddress = nftParsedEvent?.args[0];
    console.log(`BuyXApecoinWorthOfNFTs Tim3cap instance deployed to: ${nftTim3capAddress}`);
    
    // Get contract references
    const nftTim3cap = Tim3cap.attach(nftTim3capAddress);
    const nftActivityAddress = await nftTim3cap.activity();
    const nftRewardAddress = await nftTim3cap.reward();
    
    console.log(`BuyXApecoinWorthOfNFTs Activity: ${nftActivityAddress}`);
    console.log(`Token Airdrop Reward: ${nftRewardAddress}`);
    ...
    const approveTx = await rewardTokenWithBroker.approve(nftRewardAddress, approveAmount);
    await approveTx.wait();
    console.log(`Broker approved ${ethers.formatUnits(approveAmount, 18)} tokens for APE NFT purchase activity reward`);
    
    // Save instance data to a file
    const instanceData = {
      networkId: (await ethers.provider.getNetwork()).chainId.toString(),
      apeNftPurchaseInstance: {
        tim3cap: nftTim3capAddress,
        activity: nftActivityAddress,
        reward: nftRewardAddress,
        activityType: "BUY_X_APECOIN_WORTH_OF_NFTS"
      },
      brokerWallet: {
    ...
    console.log("\n=== BuyXApecoinWorthOfNFTs Instance Deployment Completed Successfully ===");
    ...
    try {
      // Check if the BuyXApecoinWorthOfNFTs implementation is correctly registered
      ...
      const buyXApeNFTsImpl = await registry.getActivityImplementation("BUY_X_APECOIN_WORTH_OF_NFTS");
      console.log(`BUY_X_APECOIN_WORTH_OF_NFTS implementation in registry: ${buyXApeNFTsImpl}`);
      ...
      const isValidCombo = await registry.isValidCombination("BUY_X_APECOIN_WORTH_OF_NFTS", "TOKEN_AIRDROP");
      console.log(`Is BUY_X_APECOIN_WORTH_OF_NFTS + TOKEN_AIRDROP valid: ${isValidCombo}`);
      ...
      // Try to validate the config data with BuyXApecoinWorthOfNFTs contract
      try {
        const buyXApeNFTsContract = BuyXApecoinWorthOfNFTs.attach(contracts.buyXApecoinWorthOfNFTsImpl);
        const isConfigValid = await buyXApeNFTsContract.validateConfig(activityConfig);
