// scripts/activities/buy_x_nfts/deploy-instance.ts
import { ethers } from "hardhat";
import * as fs from "fs";
import * as path from "path";

async function main() {
  console.log("\n=== Creating Tim3cap Instance with BuyXNFTs Activity ===");
  ...
  console.log(`- BuyXNFTs Implementation: ${contracts.buyXNftsImpl}`);
  ...
  const requiredBuyCount = 2; // Require 2 NFT purchases
  
  // Prepare activity config - 5 parameter structure for BuyXNFTs
  const nftActivityConfig = ethers.AbiCoder.defaultAbiCoder().encode(
    ["address", "uint256", "uint256", "uint256", "uint256"],
    [
      contracts.testNFT,           // NFT contract address
      requiredBuyCount,            // required buy count
      now,                         // start date
      now + 30 * 24 * 60 * 60,     // end date (30 days)
      0                            // min purchase amount (0 for no minimum)
    ]
  );
  ...
  const tokenRewardConfig = ethers.AbiCoder.defaultAbiCoder().encode(
    ...
    [
      "NFT Purchase Rewards",      // reward name
  ...
  // Create Tim3cap instance with BuyXNFTs via Factory
  console.log("\nCreating Tim3cap instance with BuyXNFTs activity via Factory...");
  
  try {
    const createNftTx = await tim3capFactory.createTim3cap(
      "BUY_X_NFTS",              // Activity type
      contracts.buyXNftsImpl,     // Activity implementation
      nftActivityConfig,          // Activity config
    ...
    console.log(`BuyXNFTs Tim3cap instance deployed to: ${nftTim3capAddress}`);
    ...
    console.log(`BuyXNFTs Activity: ${nftActivityAddress}`);
    ...
    console.log(`Broker approved ${ethers.formatUnits(approveAmount, 18)} tokens for NFT purchase activity reward`);
    ...
    console.log("\n=== BuyXNFTs Instance Deployment Completed Successfully ===");
    ...
    try {
      // Check if the BuyXNFTs implementation is correctly registered
      ...
      const buyNftsImpl = await registry.getActivityImplementation("BUY_X_NFTS");
      console.log(`BUY_X_NFTS implementation in registry: ${buyNftsImpl}`);
      ...
      const isValidCombo = await registry.isValidCombination("BUY_X_NFTS", "TOKEN_AIRDROP");
      console.log(`Is BUY_X_NFTS + TOKEN_AIRDROP valid: ${isValidCombo}`);
      ...
      // Try to validate the config data with BuyXNFTs contract
      try {
        const buyXNftsContract = BuyXNFTs.attach(contracts.buyXNftsImpl);
        const isConfigValid = await buyXNftsContract.validateConfig(nftActivityConfig);
