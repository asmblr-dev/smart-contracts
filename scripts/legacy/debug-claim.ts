...
// These addresses will come from your deploy-debug.ts script
const TIM3CAP_ADDRESS = "YOUR_TIM3CAP_ADDRESS";
const ACTIVITY_ADDRESS = "YOUR_ACTIVITY_ADDRESS";
const REWARD_ADDRESS = "YOUR_REWARD_ADDRESS";

async function main() {
  console.log("Starting debug claim process...");
  
  // Get Tim3cap contract
  const Tim3cap = await ethers.getContractFactory("Tim3cap");
  const tim3cap = Tim3cap.attach(TIM3CAP_ADDRESS);
  
  // Get Activity contract
  const HoldXNfts = await ethers.getContractFactory("HoldXNfts");
  const activity = HoldXNfts.attach(ACTIVITY_ADDRESS);
  
  // Get Reward contract
  const NFTMintReward = await ethers.getContractFactory("NFTMintReward");
  const reward = NFTMintReward.attach(REWARD_ADDRESS);
  ...
  console.log("Has user claimed:", rewardState.userHasClaimed); // Note the parameter name change
