// scripts/token_raffle/deploy-instance.ts
import { ethers } from "hardhat";
import * as fs from "fs";
import * as path from "path";

async function main() {
  console.log("\n=== Creating New Tim3cap Instance with TokenRaffleReward ===");
  ...
  const createForMode = async (automatic: any) => {
    ...
    console.log(`\n--- Creating ${mode} Distribution TokenRaffleReward ---`);
    ...
    const rewardConfig = ethers.AbiCoder.defaultAbiCoder().encode(
      ...
      [
        `Test Token Raffle (${mode})`, // raffle name
    ...
    const createTx = await tim3capFactory.createTim3cap(
      ...
      contracts.tokenRaffleRewardImpl, // Reward implementation
  ...
  // Get TokenRaffleReward contract instances
  const manualReward = TokenRaffleReward.attach(manualInstance.reward);
  const automaticReward = TokenRaffleReward.attach(automaticInstance.reward);
  ...
  console.log("Manual TokenRaffleReward Instance:");
  ...
  console.log("\nAutomatic TokenRaffleReward Instance:");
