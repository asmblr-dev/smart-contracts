// scripts/nft_airdrop/deploy-instance.ts
import { ethers } from "hardhat";
import * as fs from "fs";
import * as path from "path";

async function main() {
  console.log("\n=== Creating New Tim3cap Instance with NFTAirdropReward ===");
  ...
  console.log(`- Airdrop NFT: ${contracts.airdropNFT}`);
  ...
  // Step 2: Mint NFTs to the broker wallet for airdrop distribution
  console.log("\nMinting airdrop NFTs to broker wallet...");
  const TestNFT = await ethers.getContractFactory("TestNFT");
  const airdropNFT = TestNFT.attach(contracts.airdropNFT);
  
  // Mint 10 NFTs to the broker wallet
  for (let i = 0; i < 10; i++) {
    await airdropNFT.safeMint(brokerWallet.address);
    console.log(`Minted airdrop NFT #${i+1} to broker wallet`);
  }
  
  // Verify NFT balance of broker wallet
  const brokerNFTBalance = await airdropNFT.balanceOf(brokerWallet.address);
  console.log(`Broker wallet airdrop NFT balance: ${brokerNFTBalance}`);
  ...
  const createForMode = async (automatic: any) => {
    ...
    console.log(`\n--- Creating ${mode} Distribution NFTAirdropReward ---`);
    ...
    const totalAirdropAmount = 10; // Allow up to 10 eligible users
    
    const rewardConfig = ethers.AbiCoder.defaultAbiCoder().encode(
      ["string", "address", "uint256", "address", "bool", "uint256"],
      [
        `Test NFT Airdrop (${mode})`, // airdrop name
        contracts.airdropNFT, // NFT address
    ...
    const createTx = await tim3capFactory.createTim3cap(
      ...
      "NFT_AIRDROP", // Reward type
      contracts.nftAirdropRewardImpl, // Reward implementation
  ...
  // Step 5: Setup broker wallet to approve NFTs for both airdrop contracts
  ...
  console.log("Approving NFTs for manual airdrop contract...");
          // @ts-ignore: TypeScript doesn't know about the claim method
  const approveTx1 = await airdropNFTWithBroker.setApprovalForAll(
  ...
  console.log("Approving NFTs for automatic airdrop contract...");
          // @ts-ignore: TypeScript doesn't know about the claim method
  const approveTx2 = await airdropNFTWithBroker.setApprovalForAll(
  ...
  // Verify approvals
  const manualApproval = await airdropNFT.isApprovedForAll(brokerWallet.address, manualInstance.reward);
  console.log(`Manual reward approval status: ${manualApproval}`);
  
  const automaticApproval = await airdropNFT.isApprovedForAll(brokerWallet.address, automaticInstance.reward);
  ...
  console.log("Manual NFTAirdropReward Instance:");
  ...
  console.log("\nAutomatic NFTAirdropReward Instance:");
